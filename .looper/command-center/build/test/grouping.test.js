...
test('groups sessions by their cwd', () => {
  const groups = groupSessions([
    { key: 's1', cwd: '/home/user/fixtures/webapp', status: STATUS.WORKING },
    { key: 's2', cwd: '/home/user/fixtures/webapp', status: STATUS.IDLE },
    { key: 's3', cwd: '/home/user/fixtures/api', status: STATUS.DONE },
  ]);
  assert.equal(groups.length, 2);
  const web = groups.find((g) => g.cwd === '/home/user/fixtures/webapp');
...
test('cwd-vs-dirname conflict: grouping follows the record cwd, not the decoded dir name', () => {
  ...
  const session = {
    ...
    cwd: '/home/user/fixtures/realBeta',
    status: STATUS.WORKING,
  };
  assert.equal(resolveSessionCwd(session), '/home/user/fixtures/realBeta');
  const groups = groupSessions([session]);
  assert.equal(groups.length, 1);
  assert.equal(groups[0].cwd, '/home/user/fixtures/realBeta');
  assert.notEqual(groups[0].cwd, '/home/user/fixtures/decoyAlpha');
});
...
test('falls back to decoding the dir name only when no cwd is present', () => {
  ...
  assert.equal(resolveSessionCwd(session), '/home/user/fixtures/orphan');
});
